...
public class MilkCollectionController implements Initializable {
    ...
    public ObservableList<MilkCollection> getMilkCollection() throws SQLException, ClassNotFoundException {
        ...
        String select_query =
            "SELECT  mc.id, mc.cow_id, quantity ,period,mc.created_at from  milk_collections mc ,animals a where mc.cow_id= a.id and a.type='cow' ";
    ...
    public void afficher() throws SQLException, ClassNotFoundException {
        ...
        Callback<TableColumn<MilkCollection, String>, TableCell<MilkCollection, String>> cellFoctory = (TableColumn<
            MilkCollection,
            String
        > param) -> {
                    ...
                    if (empty) {
                        ...
                        iv_view_details.setOnMouseClicked((MouseEvent event) -> {
                            ...
                            try {
                                ...
                                controller.fetchMilkCollection(
                                    mc.getId(),
                                    mc.getCow_id(),
                                    mc.getPeriod(),
                                    mc.getQuantity(),
        ...
        if (file != null) {
            try {
                ...
                table
                    .addCell(
                        new PdfPCell(
                            new Paragraph("Cow ID", FontFactory.getFont(FontFactory.COURIER_BOLD, 12, BaseColor.BLACK))
                        )
                    )
                    .setPadding(5);
                table
                ...
                table
                    .addCell(
                        new PdfPCell(
                            new Paragraph("Period", FontFactory.getFont(FontFactory.COURIER_BOLD, 12, BaseColor.BLACK))
                        )
                    )
                    .setPadding(5);
                table
        ...
        search_input
            ...
            .addListener((observable, oldValue, newValue) -> {
                if (newValue == null || newValue.isEmpty()) {
                    ...
                    for (MilkCollection milkCollection : milkCollections) {
                        if (
                            milkCollection.getPeriod().toLowerCase().contains(newValue.toLowerCase()) ||
                            milkCollection.getCow_id().toLowerCase().contains(newValue.toLowerCase())
                        ) {
